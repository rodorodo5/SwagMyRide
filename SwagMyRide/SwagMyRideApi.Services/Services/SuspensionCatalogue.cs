...
namespace SwagMyRideApi.Services.Services
{
    public class SuspensionCatalogue: IBaseCall
    {
        ...
        {
            return _db.SuspensionCatalogue.Where(x => x.VechileTypeId == id).ToList();
        }

        public IEnumerable<Service> GetDataPerVehicleModel(int id)
        {
            return _db.SuspensionCatalogue.Where(x => x.VehicleModelId == id).ToList();
        }

        public HttpResponseMessage SaveComponent(Service service)
        {
            var data = (SwagMyRide.Data.Models.VehicleComponents.SuspensionCatalogue)service;
            if (data == null)
            {
                return new HttpResponseMessage(HttpStatusCode.NotAcceptable);
            }
            _db.SuspensionCatalogue.Add(data);
            try
            {
                _db.SaveChanges();
